...
import OrdenVentaCard from "../components/OrdenVentaCard";
import { getOrdenesVenta } from "../services/ordenesVenta.service";
import type {
  OrdenVentaEstadoFilter,
  OrdenVentaItem,
} from "../types/ordenesVenta.types";

const FILTERS: Array<{ label: string; value: OrdenVentaEstadoFilter }> = [
...
export default function OrdenesVentaScreen() {
  const [search, setSearch] = useState("");
  const [estadoFilter, setEstadoFilter] =
    useState<OrdenVentaEstadoFilter>("all");
  ...
  const query = useQuery({
    queryKey: ["ordenes-venta-mobile", selectedBodegaId],
    queryFn: () =>
      getOrdenesVenta({
        page: 1,
        limit: 1000,
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((orden: OrdenVentaItem) => {
      const matchesEstado =
        estadoFilter === "all" || orden.estado === estadoFilter;
      ...
      const bucket = [
        orden.codigo,
        orden.cliente,
        orden.numeroDocumentoCliente,
        orden.tipoDocumentoCliente,
        orden.bodega,
        orden.estado,
        orden.descripcion,
      ]
  ...
  if (query.isLoading) {
    return (
      <View style={styles.centerState}>
        ...
        <Text style={styles.stateText}>Cargando órdenes de venta...</Text>
      </View>
    );
  }
    ...
    return (
      <AppCard>
        <Text style={styles.errorTitle}>
          No fue posible cargar órdenes de venta
        </Text>
  ...
  return (
    ...
    >
      <View style={styles.header}>
        <Text style={styles.title}>Órdenes de venta</Text>
        <Text style={styles.subtitle}>
          Visualiza tus órdenes y consulta el detalle completo
        </Text>
      ...
      <View style={styles.summaryRow}>
        ...
        <AppCard style={styles.summaryCard}>
          ...
          <Text style={styles.summaryValue}>{stats.aprobadas}</Text>
        </AppCard>
      </View>
        ...
        {filteredData.length === 0 ? (
          <AppCard>
            ...
            <Text style={styles.emptyText}>
              No encontramos órdenes de venta para ese criterio.
            </Text>
          </AppCard>
        ) : (
          filteredData.map((orden) => (
            <OrdenVentaCard key={orden.id} orden={orden} />
          ))
        )}
      </View>
    </ScrollView>
