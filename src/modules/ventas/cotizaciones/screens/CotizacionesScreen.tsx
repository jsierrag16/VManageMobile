...
import type {
  CotizacionEstadoFilter,
  CotizacionItem,
} from "../types/cotizaciones.types";

const FILTERS: Array<{ label: string; value: CotizacionEstadoFilter }> = [
...
export default function CotizacionesScreen() {
  ...
  const [estadoFilter, setEstadoFilter] =
    useState<CotizacionEstadoFilter>("all");
  ...
  const query = useQuery({
    queryKey: ["cotizaciones-mobile", selectedBodegaId],
    queryFn: () =>
      getCotizaciones({
        page: 1,
        limit: 1000,
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((cotizacion: CotizacionItem) => {
      const matchesEstado =
        estadoFilter === "all" || cotizacion.estado === estadoFilter;
      ...
      const bucket = [
        cotizacion.codigo,
        cotizacion.cliente,
        cotizacion.numeroDocumentoCliente,
        cotizacion.tipoDocumentoCliente,
        cotizacion.bodega,
        cotizacion.estado,
  ...
  if (query.isLoading) {
    return (
      <View style={styles.centerState}>
        ...
        <Text style={styles.stateText}>Cargando cotizaciones...</Text>
      </View>
    );
  }
  ...
  if (query.isError) {
    return (
      <AppCard>
        <Text style={styles.errorTitle}>No fue posible cargar cotizaciones</Text>
    ...
    >
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>
          Visualiza tus cotizaciones y consulta el detalle completo
        </Text>
        ...
        {filteredData.length === 0 ? (
          <AppCard>
            ...
            <Text style={styles.emptyText}>
              No encontramos cotizaciones para ese criterio.
            </Text>
          </AppCard>
        ) : (
