...
import type { OrdenVentaItem } from "../types/ordenesVenta.types";

type Props = {
  orden: OrdenVentaItem;
};
...
export default function OrdenVentaCard({ orden }: Props) {
  const [expanded, setExpanded] = useState(false);

  const detailQuery = useQuery<OrdenVentaDetail>({
    queryKey: ["orden-venta-detail-mobile", orden.id],
    queryFn: () => getOrdenVentaById(orden.id),
    enabled: expanded,
    staleTime: 30000,
  });

  const estadoStyles = useMemo(
    () => getEstadoStyles(orden.estado),
    [orden.estado]
  );
      ...
      <Pressable onPress={() => setExpanded((prev) => !prev)}>
        ...
        <View style={styles.metaRow}>
          <View style={styles.metaChip}>
            ...
            />
            <Text style={styles.metaChipText}>{formatDate(orden.fecha)}</Text>
          </View>
          ...
          <View style={styles.metaChip}>
            ...
            <Text style={styles.metaChipText}>
              Vence: {formatDate(orden.fechaVencimiento)}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.metaRow}>
          ...
          <View style={styles.metaChip}>
            ...
            />
            <Text style={styles.metaChipText}>
              {orden.totalItems} item{orden.totalItems === 1 ? "" : "s"}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.bottomRow}>
          <View style={styles.bottomInfo}>
            ...
            <Text style={styles.bottomValue}>{formatCurrency(orden.total)}</Text>
          </View>
            ...
            <>
              <View style={styles.detailsGrid}>
                <DetailRow
                  icon="person-outline"
                  label="Cliente"
                  value={detailData?.cliente || orden.cliente}
                />
                ...
                <DetailRow
                  ...
                  value={
                    [
                      detailData?.tipoDocumentoCliente || orden.tipoDocumentoCliente,
                      detailData?.numeroDocumentoCliente || orden.numeroDocumentoCliente,
                    ]
                      .filter(Boolean)
                      .join(": ") || "No registrado"
                  }
                ...
                <DetailRow
                  ...
                  label="Bodega"
                  value={detailData?.bodega || orden.bodega}
                />
                ...
                {(detailData?.productos ?? []).length === 0 ? (
                  <View style={styles.emptyProductsBox}>
                    <Text style={styles.emptyProductsText}>
                      Esta orden no tiene productos visibles en el detalle.
                    </Text>
                  </View>
                ) : (
              ...
              <View style={styles.resumeSection}>
                <View style={styles.resumeRow}>
                  ...
                  <Text style={styles.resumeValue}>
                    {formatCurrency(detailData?.subtotal ?? orden.subtotal)}
                  </Text>
                </View>
                ...
                <View style={styles.resumeRow}>
                  ...
                  <Text style={styles.resumeValue}>
                    {formatCurrency(detailData?.totalIva ?? orden.totalIva)}
                  </Text>
                </View>
                ...
                <View style={[styles.resumeRow, styles.resumeRowStrong]}>
                  ...
                  <Text style={styles.resumeValueStrong}>
                    {formatCurrency(detailData?.total ?? orden.total)}
                  </Text>
                </View>
              </View>
            </>
