...
import {
  getRemisionVentaById,
  type RemisionVentaDetail,
} from "../services/remisionesVenta.service";
import type { RemisionVentaItem } from "../types/remisionesVenta.types";

type Props = {
  remision: RemisionVentaItem;
};
...
function getEstadoStyles(value?: string | null) {
  ...
  switch (normalized) {
    ...
    case "despachado":
      return {
        badge: styles.statusDispatched,
        text: styles.statusTextDispatched,
        label: "Despachado",
      };
    case "entregado":
      return {
...
export default function RemisionVentaCard({ remision }: Props) {
  const [expanded, setExpanded] = useState(false);

  const detailQuery = useQuery<RemisionVentaDetail>({
    queryKey: ["remision-venta-detail-mobile", remision.id],
    queryFn: () => getRemisionVentaById(remision.id),
    enabled: expanded,
    staleTime: 30000,
  });

  const estadoStyles = useMemo(
    () => getEstadoStyles(remision.estado),
    [remision.estado]
  );
      ...
      <Pressable onPress={() => setExpanded((prev) => !prev)}>
        <View style={styles.topRow}>
          <View style={styles.headerContent}>
            ...
            <Text style={styles.name} numberOfLines={2}>
              {remision.cliente || "Cliente no disponible"}
            </Text>
          </View>
        ...
        <View style={styles.metaRow}>
          <View style={styles.metaChip}>
            ...
            />
            <Text style={styles.metaChipText}>{formatDate(remision.fecha)}</Text>
          </View>
          ...
          <View style={styles.metaChip}>
            ...
            <Text style={styles.metaChipText}>
              Entrega: {formatDate(remision.fechaEntrega)}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.metaRow}>
          ...
          <View style={styles.metaChip}>
            ...
            />
            <Text style={styles.metaChipText}>
              {remision.totalItems} item{remision.totalItems === 1 ? "" : "s"}
            </Text>
          </View>
        </View>
            ...
            <>
              <View style={styles.detailsGrid}>
                <DetailRow
                  icon="person-outline"
                  label="Cliente"
                  value={detailData?.cliente || remision.cliente}
                />
                ...
                <DetailRow
                  ...
                  value={
                    [
                      detailData?.tipoDocumentoCliente || remision.tipoDocumentoCliente,
                      detailData?.numeroDocumentoCliente || remision.numeroDocumentoCliente,
                    ]
                      .filter(Boolean)
                      .join(": ") || "No registrado"
                  }
                ...
                <DetailRow
                  ...
                  label="Bodega"
                  value={detailData?.bodega || remision.bodega}
                />
                ...
                {(detailData?.productos ?? []).length === 0 ? (
                  <View style={styles.emptyProductsBox}>
                    <Text style={styles.emptyProductsText}>
                      Esta remisión no tiene productos visibles en el detalle.
                    </Text>
                  </View>
                ) : (
              ...
              <View style={styles.resumeSection}>
                <View style={styles.resumeRow}>
                  ...
                  <Text style={styles.resumeValue}>
                    {formatCurrency(detailData?.subtotal ?? remision.subtotal)}
                  </Text>
                </View>
                ...
                <View style={styles.resumeRow}>
                  ...
                  <Text style={styles.resumeValue}>
                    {formatCurrency(detailData?.totalIva ?? remision.totalIva)}
                  </Text>
                </View>
                ...
                <View style={[styles.resumeRow, styles.resumeRowStrong]}>
                  ...
                  <Text style={styles.resumeValueStrong}>
                    {formatCurrency(detailData?.total ?? remision.total)}
                  </Text>
                </View>
              </View>
            </>
